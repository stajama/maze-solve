def naiverec(matrix, start=None, end=None, been=None, pathlist=None):
    ...
    for coord in [(start[0] - 1, start[1]), (start[0] + 1, start[1]),
        ...
        if not matrix[coord[0]][coord[1]] == 0 and not coord[0] < 0 and\
                      not coord[0] >= len(matrix) and not coord[1] < 0 and\
                      not coord[1] >= len(matrix[0]) and coord not in been:
...
def naiveimp(matrix):
    ...
    def isgoodcoord(coordinate, matr, been):
        ...
        if not coordinate[0] < 0 and not coordinate[0] > len(matr) - 1 and\
           not coordinate[1] < 0 and not coordinate[1] >= len(matr[0]) and\
    ...
    while start != end:
        ...
        for coord in checks:
            ...
            if isgoodcoord(coord, matrix, beenlist):
                ...
                start = coord
                '''print('bing', [des for des in NSEWget(coord) if/
                                  isgoodcoord(des, matrix, [])==True])'''
